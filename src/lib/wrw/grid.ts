...
/* Every CCTV monitor is rendered at this one uniform on-screen picture size
   ...
   For a hand-placed, messy look each monitor is yaw-rotated a little
   differently (rotY, no pitch/roll) and nudged forward (toward the room, −x)
   and sideways (±z) by varying small amounts off the clean grid — so they
   stagger in depth and don't line up perfectly. Y is left on the grid so the
   bottom row stays flush on the desk.

   Base layout: the five middle monitors form a pyramid (1 straddles 2/3, with
   4/5 beneath), and 6/7 are a stacked pair off to the right.
       [1]
     [2][3]      [6]
     [4][5]      [7]                                                          */
